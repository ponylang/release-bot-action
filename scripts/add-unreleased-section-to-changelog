...
print(INFO + "Adding new 'unreleased' section to CHANGELOG.md" + ENDC)
try:
    subprocess.run(['changelog-tool', 'unreleased', '-e'],
        capture_output=True,
        text=True,
        check=True)
except subprocess.CalledProcessError as e:
    print(ERROR + "Unable to add unreleased section." + ENDC)
    print(ERROR + e.stdout + ENDC)
    print(ERROR + e.stderr + ENDC)

print(INFO + "Commiting CHANGELOG.md change" + ENDC)
git.add('CHANGELOG.md')
if not git.status("-s"):
    print(INFO + "No changes. That ain't right. Exiting." + ENDC)
    sys.exit(1)
git.commit('-m',
    f'Add unreleased section to CHANGELOG post {version} release')
